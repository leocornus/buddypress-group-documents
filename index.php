...
Tested up to: WP 3.0-beta2 , BuddyPress 1.2.3
...
define ( 'BP_GROUP_DOCUMENTS_VERSION', '0.3.5' );
 ...
 * Also add options item under group "admin" 
 */
...
function bp_group_documents_display_content() {
	...
	<?php if( $template->document_list && count($template->document_list >= 1) ) { ?>
		...
		foreach( $template->document_list as $document_params ) {
			...
			<span class="group-documents-meta"><?php printf( __( 'Uploaded by %s on %s', 'bp-group-documents'),bp_core_get_userlink($document->user_id),date_i18n( 'Y. m. j.', $document->created_ts )); ?></span>
